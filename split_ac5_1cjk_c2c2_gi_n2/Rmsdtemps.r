...
library(ggplot2)
argfilepath="/home/tamans/home2/split_ac5_1cjk_c2c2_gi_n2/arg_file_r.txt"
lskip<-list()
file_name<-list()
j=0
k=0
...
for (name in rownames(argv))
{
	...
	if (grepl("chain_name",name)==TRUE)
	{
		...
		if(is.na(lskip[k]) !=TRUE)
		{
			...
			main<-paste("RMSD time series of domain",argv[name,1],"of AC5+Gi+ATP",sep=" ")
			rmsd$V2<-rmsd$V2*10
			...
			p<-p+geom_vline(aes(xintercept=1.26),color="blue", linetype="dashed", size=1)+geom_vline(aes(xintercept=0.665),color="blue", linetype="dashed", size=1)+geom_vline(aes(xintercept=0.92),color="blue", linetype="dashed", size=1)#to add vertical ligne in function of the xvalue (xintercept), here it's an example for A chain of AC5 in the complexe AC5+Gi
			print(p)
			dev.off()
			moyenne<-mean(rmsd$V2)
