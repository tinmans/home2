...
library(ggplot2)
argfilepath="/home/tamans/home2/ac5_1azs_c2c2_gi_n2_snapshot/arg_file_part2_r.txt"
lskip<-list()
file_name<-list()
j=0
k=0
...
for (name in rownames(argv))
{
	...
	if (grepl("chain_name",name)==TRUE)
	{
		...
		if(is.na(lskip[k]) !=TRUE)
		{
			...
			name_file_pdf<-paste("rmsd_part2_chain",argv[name,1],".pdf",sep="")
			...
			dev.off()
			moyenne<-mean(rmsd$V2)
			name_file_txt<-paste("rmsd_part2_chain",argv[name,1],"_mean.txt",sep="")
			sink(name_file_txt)
			cat(moyenne)
			sink()
		}
