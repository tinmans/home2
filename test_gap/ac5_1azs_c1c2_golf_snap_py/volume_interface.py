...
filetrj=open('snap_ac5_1azs_c1c2_golfpbcok0.pdb','r')
k=0
...
for line in filetrj:
    ...
    if line[0:5]=='MODEL':
        ...
        filepdb.close()
	subprocess.call('chimera -n interface.py',shell=True)
        file1=open('a.pdb','r')
        fileint=open('int.pdb','w')
        lll=0
        for line2 in file1:
            if(line2[0:4]=='ATOM' or line2[0:6]=='HETATM'): #si on a a faire a atom ou HETAM alors prendre les infos
                lll=lll+1
               #print line2
                natom=line2[6:12] #recupere le numero de atome de lacide amine
                fileint.write(line2) #ecrit toute la ligne dans le fichier
                if(lll==1): #si on est sur le premier atome alors stoker linfo
                    natom1s=natom
        natom1e=natom
        #file1.close()
	file1=open('b.pdb','r')
	...
	subprocess.call('surfnet',shell=True)
